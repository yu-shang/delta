...
double get_pvalue (continuous a, int b)
{
	int i =0;
	double one = 1, pvalue=0;
	double poisson=one/exp(a);
	for (i=0;i<b+300;i++)
	{
		if (i>(b-1)) 
...
static void block_init(Edge *e, Block *b, 
                     ...
                     int *components, struct dyStack *allincluster, double *pvalues)
{
	int i,score,top;
...
{	
	...
	for (i=0; i<dsSize(b->genes); i++)
	{
		...
		fprintf(fw, "\n"); /* fputc('\n',fw); */
		if (i == b->block_rows_pre -1) fprintf(fw, "\n"); /* fputc('\n',fw); */ /* add a blank line for negative */
	}
	fprintf(fw, "\n"); /* fputc('\n',fw); */
}
...
int init_qubic( double *r_data, char **r_rowsnames, char **r_colsnames, int *r_rows, int *r_cols, char *tfile, double *rq, double *rc, double *rf, int *rk, int *rr, int *ro, int *rd )
{
  ...
  memset(bb, -1, USHRT_MAX*sizeof(*bb));
	...
	if(po->IS_DISCRETE)
	{
		...
		/* printf("Discretized data contains %d classes with charset [ ", sigma); */ /*printf*/;
		for(i = 0 ; i < sigma; i++)
